...
Then run changesets' init command to generate a changeset config:
...
To automate the process, you can use `changeset version` with GitHub actions. The action will detect when changeset files arrive in the `main` branch, and then open a new PR listing all the packages with bumped versions. The PR will update itself every time a new changeset file arrives in `main`. Once merged, the packages will be updated and you can decide whether to publish or not by adding the `publish` property to the action.

### Add a publish script

Add a new script `ci:publish` which executes `pnpm publish -r`. It will publish to the registry once the PR is opened by `changeset version`. If the package is public and scoped, adding `--access=public` may be necessary to prevent npm rejecting the publish.
